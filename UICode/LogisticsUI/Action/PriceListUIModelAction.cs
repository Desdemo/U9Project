...
{
	public partial class PriceListUIModelAction : BaseAction
	{
		private static readonly ILogger logger = LoggerManager.GetLogger(typeof(PriceListUIModelAction));
		//强类型化基类Model属性.
		public new PriceListUIModelModel CurrentModel 
		{
			get {
				return (PriceListUIModelModel)base.CurrentModel;
			}
			set {
		...
		public PriceListUIModelAction(IPart part) : base(part)
		{
		}
		//Model参数的构造,用于测试用例.
		public PriceListUIModelAction(PriceListUIModelModel model) : base(model)
		{
		}
		/// <summary>
		/// Help: 
