...
namespace UFIDA.U9.Cust.BLT.ShipPlan.LogisticsUI
{
    public partial class PriceTypeUIFormWebPart
    {
        ...
        public void AfterCreateChildControls()
        {
			...
			//查询按钮设置
			PDFormMessage.ShowConfirmDialog(this.Page, "5089809c-f207-4346-aebe-1b12fb68b033", "580", "408",
		...
		private void AutoCopyRow1()
		{
			this.DataGrid5.AllowCopyRow = true;

			AssociationControl assoCtrl = new AssociationControl();

			assoCtrl.SourceServerControl = this.DataGrid5;

			assoCtrl.SourceControl.EventName = "OnAfterRowCopyed";


			ClientCallBackFrm cBFrm = new ClientCallBackFrm();

			cBFrm.IsLightClientCallback = false;

			cBFrm.ParameterControls.Add(this.DataGrid5);

			//cBFrm.DoCustomerAction += handler;//事件处理方法
		...
		private void AllotQtyChangedCallBack()
		{
			...
			//CallBack处理方案
			((IUFClientAssoGrid)gridCellDataChangedASC.SourceControl).FireEventCols.Add("Start");
			((IUFClientAssoGrid)gridCellDataChangedASC.SourceControl).FireEventCols.Add("Cutoff");
		...
		private object AllotQtyChange(CustomerActionEventArgs args)
		{
			...
			decimal Start = decimal.Parse(table["Start"].ToString()); //开始
			decimal Cutoff = decimal.Parse(table["Cutoff"].ToString());  //截止
			decimal LineTotal = Cutoff - Start;
			//合计
